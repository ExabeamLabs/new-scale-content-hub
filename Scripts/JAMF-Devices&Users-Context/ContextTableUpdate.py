...
def authenticate_with_exabeam(exabeam_url):
    ...
    try:
        response = requests.post(auth_url, headers={'Content-Type': 'application/json'}, data=json.dumps(payload), verify=True)
...
def get_context_table_id(token, exabeam_url, table_name):
    ...
    try:
        response = requests.get(f"{exabeam_url}/context-management/v1/tables", headers=headers, verify=True)
        response.raise_for_status()
