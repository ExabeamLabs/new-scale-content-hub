...
def main(device_name: str) -> None:
    ...
    if results.get("value"):
        ...
        print(f"\nAbout to isolate {device_dns} (ID: {device_id})")

        # Construct the isolation API endpoint
        isolate_url = f"https://api.securitycenter.microsoft.com/api/machines/{device_id}/isolate"

        # Payload: comment + isolation type ("Full" isolates network completely)
        payload = {"Comment": "isolation via script", "IsolationType": "Full"}

        # POST request to initiate isolation
        resp = requests.post(isolate_url, headers={**headers, "Content-Type": "application/json"}, json=payload)
